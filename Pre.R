...
#determine days to report to hospital + new column
...
#took out "currently columns" not much value, too little observations
df2<-df1[ -c(11:13) ]
head(df2)
summary(df2)
...
#take out negative days to reports
df3 <- df2
df3$Onset.Report[df3$Onset.Report<0] <- 0


#take out observations not confirmed covid
df4<-df3
summary(df4)
...
#can use to later as test, currently still in hospital, active cases
...
#join demographic with 2016 census data
